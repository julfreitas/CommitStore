...
const OrderSummary = ({ totalPrice, items }) => {
  ...
  const handleCouponCode = async (event) => {
    ...
    // Lógica para validar/aplicar cupom
  }
  ...
  return (
    <div className='w-full max-w-lg lg:max-w-[340px] bg-slate-50/30 border border-slate-200 text-slate-500 text-sm rounded-xl p-7'>
      <h2 className='text-xl font-medium text-slate-600'>Resumo de Pagamento</h2>
      <p className='text-slate-400 text-xs my-4'>Método de pagamento</p>
      <div className='flex gap-2 items-center'>
        <input type="radio" id="COD" onChange={() => setPaymentMethod('COD')} checked={paymentMethod === 'COD'} className='accent-gray-500' />
        <label htmlFor="COD" className='cursor-pointer'>Pagamento na Entrega (COD)</label>
      </div>
      <div className='flex gap-2 items-center mt-1'>
        <input type="radio" id="STRIPE" name='payment' onChange={() => setPaymentMethod('STRIPE')} checked={paymentMethod === 'STRIPE'} className='accent-gray-500' />
        <label htmlFor="STRIPE" className='cursor-pointer'>Pagamento via Stripe</label>
      </div>
          ...
          selectedAddress ? (
            ...
            <div>
              ...
              <button className='flex items-center gap-1 text-slate-600 mt-1' onClick={() => setShowAddressModal(true)} >
                Adicionar endereço <PlusIcon size={18} />
              </button>
            </div>
          )
        }
      ...
      <button onClick={e => toast.promise(handlePlaceOrder(e), { loading: 'Finalizando pedido...' })} className='w-full bg-slate-700 text-white py-2.5 rounded hover:bg-slate-900 active:scale-95 transition-all'>
        Finalizar Pedido
      </button>
