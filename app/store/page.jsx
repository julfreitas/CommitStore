...
export default function Dashboard() {
  ...
  const dashboardCardsData = [
    { title: 'Total de produtos', value: dashboardData.totalProducts, icon: ShoppingBasketIcon },
    { title: 'Total de ganhos', value: currency + dashboardData.totalEarnings, icon: CircleDollarSignIcon },
    { title: 'Total de pedidos', value: dashboardData.totalOrders, icon: TagsIcon },
    { title: 'Total de avaliações', value: dashboardData.ratings.length, icon: StarIcon },
  ]
  ...
  return (
    <div className=" text-slate-500 mb-28">
      <h1 className="text-2xl">Painel do <span className="text-slate-800 font-medium">Vendedor</span></h1>
          ...
          dashboardData.ratings.map((review, index) => (
            <div key={index} className="flex max-sm:flex-col gap-5 sm:items-center justify-between py-6 border-b border-slate-200 text-sm text-slate-600 max-w-4xl">
              ...
              <div className="flex flex-col justify-between gap-6 sm:items-end">
                ...
                <button onClick={() => router.push(`/product/${review.product.id}`)} className="bg-slate-100 px-5 py-2 hover:bg-slate-200 rounded transition-all">Ver produto</button>
              </div>
            </div>
          ))
        }
